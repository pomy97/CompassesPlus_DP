...
execute unless score #itemCount hu.temp = #slot hu.temp \
        store result storage hunters:temp data.reward.slot int 1 \
        run scoreboard players get #slot hu.temp
execute unless score #itemCount hu.temp = #slot hu.temp \
        run return run function hunters:item/bounty_compass/craft_bounty/store_container_data with storage hunters:temp data.reward